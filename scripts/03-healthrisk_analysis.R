...
# Then, run each Option and its respective 'data_site'

### Primary model 
########################################


# OPTION 1 - Not grouped
portion = 1:7
...
data_site <- final_data_3 %>%
  filter(SEX == sex) ####### Change here, to specify the analysis by SEX


########################################


# OPTION 3 - Grouped only by CID
...
data_site <- final_data_3 %>%
  filter(CID == outcome) ####### Change here, to specify the analysis by CID
...
# OPTION 4 - Grouped only by CID and SEX
...
for (j in 0:4) {
  ...
  ####### Change here, Factor number All (1:7) // All Female (8:14) // All Male (15:21)
  ####### Change here, Factor number Resp (22:28) // Resp Female (29:35) // Resp Male (36:42)
  ####### Change here, Factor number Circul (43:49) // Circul Female (50:56) // Circul Male (57:63)
  ...
  # WD (1 day)
  vars <- c("wd", "Lag1_WD")
  data_site$WD_avg_1day <- rowMeans(data_site[ , vars], na.rm = TRUE)
  # WD (2 days)
  vars <- c("wd", "Lag1_WD", "Lag2_WD")
  data_site$WD_avg_2days <- rowMeans(data_site[ , vars], na.rm = TRUE)
  # WD (3 days)
  vars <- c("wd", "Lag1_WD", "Lag2_WD", "Lag3_WD")
  data_site$WD_avg_3days <- rowMeans(data_site[ , vars], na.rm = TRUE)
  # WD (4 days)
  vars <- c("wd", "Lag1_WD", "Lag2_WD", "Lag3_WD", "Lag4_WD")
  data_site$WD_avg_4days <- rowMeans(data_site[ , vars], na.rm = TRUE)
  # WD (5 days)
  vars <- c("wd", "Lag1_WD", "Lag2_WD", "Lag3_WD", "Lag4_WD", "Lag5_WD")
  data_site$WD_avg_5days <- rowMeans(data_site[ , vars], na.rm = TRUE)
  ...
  data <- data %>%
    #rowwise() %>%
    mutate(total = data[[paste0("F1", coluna)]] +
             data[[paste0("F2", coluna)]] + data[[paste0("F3", coluna)]] + data[[paste0("F4", coluna)]] +
             data[[paste0("F5", coluna)]] + data[[paste0("F6", coluna)]] + data[[paste0("F7", coluna)]],
           prop1 = data[[paste0("F1", coluna)]]/data[[paste0("PM25", coluna)]],
           prop2 = data[[paste0("F2", coluna)]]/data[[paste0("PM25", coluna)]],
           prop3 = data[[paste0("F3", coluna)]]/data[[paste0("PM25", coluna)]],
           prop4 = data[[paste0("F4", coluna)]]/data[[paste0("PM25", coluna)]],
           prop5 = data[[paste0("F5", coluna)]]/data[[paste0("PM25", coluna)]],
           prop6 = data[[paste0("F6", coluna)]]/data[[paste0("PM25", coluna)]],
           prop7 = data[[paste0("F7", coluna)]]/data[[paste0("PM25", coluna)]])
  
  
  
  strata_config = "site_DOW_Month_Year_qtil.75"
  # strata_config = "site_DOW_Month_Year_qtil.75_propPM"
  
  for (i in portion) {
    
    if (i == 1 | i == 8 | i == 15 | i == 22 | i == 29 | i == 36 | i == 43 | i == 50 | i == 57) {
      source = "Mineral dust and soil ressuspension"
      #data$case <- ifelse(data$prop1 >= 1/7, 1 , 0)  
      quantile_F1 = as.numeric(quantile(data[[paste0("F1", coluna)]], c(.75), na.rm = T))
      data$case <- ifelse(data[[paste0("F1", coluna)]] >= quantile_F1, 1 , 0) 
      #quantile_F1PM = as.numeric(quantile(data$prop1, c(.5), na.rm = T))
      #data$case <- ifelse(data[[paste0("F1", coluna)]] >= quantile_F1  &
      #                              data$prop1 >= quantile_F1PM, 1 , 0) 
      
    } else if (i == 2 | i == 9 | i == 16 | i == 23 | i == 30 | i == 37 | i == 44 | i == 51 | i == 58) {
      source = "Aged sea salt"
      #data$case <- ifelse(data$prop2 >= 1/7, 1 , 0) 
      quantile_F2 = as.numeric(quantile(data[[paste0("F2", coluna)]], c(.75), na.rm = T))
      data$case <- ifelse(data[[paste0("F2", coluna)]] >= quantile_F2, 1 , 0) 
      #data$case <- ifelse(data[[paste0("F2", coluna)]] >= quantile_F2 &
      #                               data$prop2 >= 1/7 , 1 , 0) 
      
    } else if (i == 3 | i == 10 | i == 17 | i == 24 | i == 31 | i == 38 | i == 45 | i == 52 | i == 59) {
      source = "Secondary nitrate"
      #data$case <- ifelse(data$prop3 >= 1/7, 1 , 0)
      quantile_F3 = as.numeric(quantile(data[[paste0("F3", coluna)]], c(.75), na.rm = T))
      data$case <- ifelse(data[[paste0("F3", coluna)]] >= quantile_F3, 1 , 0) 
      #data$case <- ifelse(data[[paste0("F3", coluna)]] >= quantile_F3  &
      #                               data$prop3 >= 1/7, 1 , 0) 
      
    } else if (i == 4 | i == 11 | i == 18 | i == 25 | i == 32 | i == 39 | i == 46 | i == 53 | i == 60) {
      source = "Fresh sea salt mixed with aged emissions"
      #data$case <- ifelse(data$prop4 >= 1/7, 1 , 0)  
      quantile_F4 = as.numeric(quantile(data[[paste0("F4", coluna)]], c(.75), na.rm = T))
      data$case <- ifelse(data[[paste0("F4", coluna)]] >= quantile_F4, 1 , 0) 
      #data$case <- ifelse(data[[paste0("F4", coluna)]] >= quantile_F4  &
      #                               data$prop4 >= 1/7, 1 , 0) 
      ...
      #data$case <- ifelse(data$prop5 >= 1/7, 1 , 0)  
      quantile_F5 = as.numeric(quantile(data[[paste0("F5", coluna)]], c(.75), na.rm = T))
      data$case <- ifelse(data[[paste0("F5", coluna)]] >= quantile_F5  &
                            data$prop5 >= 1/7, 1 , 0) 
      summary(data)
    ...
    #model_clogit <- clogit(case ~ PM2.5 + temp + umid + prec +
    #wd + ws + 
    #QT_DIARIAS + strata(strata_id),
    #data)
    ...
    results.temp[i+j*63,5]<-"Primary analysis"
    ...
    summary(avg_concentrations)
    
    ### Save the results in a Table
    ################################################
    colnames(results.temp)<-c("Strata", "Outcome", "Group", "Source", "Model", "Moving Average", "Controls(n)", "Cases(n)", 
                              "Odds Ratio", "Lower 95%CI", "Upper 95%CI", 
                              "% increase", "% increase Lower 95%CI", "% increase Upper 95%CI",
                              "AIC", "BIC")
    
    
    results_temp_lag <- results.temp
...
write_csv(results_temp_lag, 
          "./output/03-results_cross_sourceqtil.csv")
...
##########################################################################################################
results_temp_lag_cols <- results_temp_lag

colnames(results_temp_lag_cols)<-c("Region", "Outcome", "Group", "Source", "Model", "Moving_Average", "Controls(n)", 
...
results_temp_lag_cols %>%
  ...
  scale_y_continuous(limits = c(min(results_temp_lag_cols$increaseLower95CI, na.rm = T)-10, 100),
