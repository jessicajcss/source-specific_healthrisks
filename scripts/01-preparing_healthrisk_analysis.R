...
factors <- read_csv("./output/02-contribution_factor.csv") %>%
  melt(id = c("site", "date")) %>%
  rename("fator" = "variable",
         "total" = "value") %>%
  rowwise() %>%                   
  mutate(source = case_when(fator == 'Factor.1' & site == 'MIC' ~ "mineral",
                            fator == 'Factor.2' & site == 'MIC' ~ "agedss",
                            fator == 'Factor.3' & site == 'MIC' ~ "secnitrate",
                            fator == 'Factor.4' & site == 'MIC' ~ "freshss",
                            fator == 'Factor.5' & site == 'MIC' ~ "industrial",
                            fator == 'Factor.6' & site == 'MIC' ~ "secsulphate",
                            fator == 'Factor.7' & site == 'MIC' ~ "combustion",
                            fator == 'Factor.1' & site == 'NID' ~ "freshss",
                            fator == 'Factor.2' & site == 'NID' ~ "secsulphate",
                            fator == 'Factor.3' & site == 'NID' ~ "agedss",
                            fator == 'Factor.4' & site == 'NID' ~ "industrial",
                            fator == 'Factor.5' & site == 'NID' ~ "combustion",
                            fator == 'Factor.6' & site == 'NID' ~ "secnitrate",
                            fator == 'Factor.7' & site == 'NID' ~ "mineral",
                            TRUE ~ NA)) %>%
...
summary(source_samples)
#source_samples <- rename(source_samples, 
#                         replace = c(date = "date.start"))

daily_source_samples <- left_join(output, 
                                  source_samples, 
                                  by = c("date.start", "date.end"))
head(daily_source_samples)
...
#rename(replace = c(DT_INTER = "date"))
...
myvars <- c("site",                                                 # Location variables
            ...
            "date", "Year", "Month", "Day", "DOW",                  # Temporal variables   
